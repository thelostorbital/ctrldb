...
- External processes must be invoked with structured arguments, bounded
  ...
  Only `internal/gcp/process.go` may import `os/exec`; its `syscall` exception
  is limited to terminating the isolated process group on cancellation. The
  repository-wide architecture check must continue to reject those imports in
  every other file, including aliases and copies, and must reject cgo and its
  compiler directives, Go assembly, SWIG inputs, precompiled native objects,
  dynamic Go plugins, `go:linkname`, and `os.StartProcess` everywhere.
