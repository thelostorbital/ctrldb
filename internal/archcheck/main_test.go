...
func TestFindArchitectureViolations(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "approved process boundary imports",
			filename: "internal/gcp/process.go",
			source:   "package gcp\nimport (\n\t\"os/exec\"\n\t\"syscall\"\n)\nvar _ = exec.CommandContext\nvar _ = syscall.Kill\n",
		},
		{
			name:     "copied process boundary",
			filename: "internal/gcp/process_copy.go",
			source:   "package gcp\nimport _ \"os/exec\"\n",
			want:     1,
		},
		{
			name:     "moved process boundary",
			filename: "internal/provider/process.go",
			source:   "package provider\nimport _ \"os/exec\"\n",
			want:     1,
		},
		{
			name:     "aliased process import outside boundary",
			filename: "internal/gcp/alternate.go",
			source:   "package gcp\nimport process \"os/exec\"\nvar _ = process.CommandContext\n",
			want:     1,
		},
		{
			name:     "aliased process import inside boundary",
			filename: "internal/gcp/process.go",
			source:   "package gcp\nimport process \"os/exec\"\nvar _ = process.CommandContext\n",
			want:     1,
		...
		{
			name:     "blank syscall import inside boundary",
			filename: "internal/gcp/process.go",
			source:   "package gcp\nimport _ \"syscall\"\n",
			want:     1,
			message:  "standard import name",
