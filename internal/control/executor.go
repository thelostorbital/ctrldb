...
type StorageStepPort interface {
	AuditBucketPort
	// AuthorizedStep returns the envelope binding hash, operation ID, and
	// step ID the port was authorized for; the executor refuses a port that
	// is not bound to the executing envelope and intent.
	AuthorizedStep() (envelopeBindingSHA256, operationID, stepID string)
	CreateControlBucket(ctx context.Context, identity BucketIdentity) error
	EnableVersioning(ctx context.Context, identity BucketIdentity) error
...
// LockRoundTripInput is the operator's explicit K5 identity; hostname and pid
// are caller-supplied observations, never inferred here.
type LockRoundTripInput struct {
	Holder     LockHolder
	CLIVersion string
}

// StepContext carries the durable facts the executor cannot derive itself:
// the sealed step IDs the journal records as DONE (dependency proof) and the
// K5 holder input.
type StepContext struct {
	CompletedSteps []string
	Lock           LockRoundTripInput
}
...
func (executor *StorageExecutor) Execute(ctx context.Context, envelope BootstrapEnvelopeV1, intent bootstrap.StepIntent, port StorageStepPort, step StepContext) (StepOutcome, error) {
	...
	if err := admitStep(envelope, intent, step.CompletedSteps); err != nil {
		return StepOutcome{}, err
	}
	binding, operationID, stepID := port.AuthorizedStep()
	if binding != envelope.EnvelopeBindingSHA256() || operationID != envelope.OperationID() || stepID != intent.StepID {
		return StepOutcome{}, fmt.Errorf("%w: port is not authorized for this envelope and step", ErrStepNotAdmitted)
	}
	...
	case bootstrap.IntentControlBucket:
		return executor.controlBucket(ctx, envelope, port, controlBucket, outcome)
	...
	case bootstrap.IntentSeedControl:
		return executor.seedControl(ctx, envelope, port, controlBucket, outcome)
...
func admitStep(envelope BootstrapEnvelopeV1, intent bootstrap.StepIntent, completed []string) error {
	...
	for _, dependency := range intent.Dependencies {
		if _, ok := done[dependency]; !ok {
			return fmt.Errorf("%w: dependency %s is not durably complete", ErrStepNotAdmitted, dependency)
		}
	}
	switch intent.Kind {
...
// controlBucket creates or converges the control bucket. An existing bucket
// is accepted only when the local handoff chain records that this operation
// claimed and created it; any other existing bucket blocks (the sealed
// expectation was absence and no operation-bound marker proves ownership).
func (executor *StorageExecutor) controlBucket(ctx context.Context, envelope BootstrapEnvelopeV1, port StorageStepPort, identity BucketIdentity, outcome StepOutcome) (StepOutcome, error) {
	handoff, err := NewAuditHandoff(executor.directory, port, executor.clock)
	if err != nil {
		return outcome, err
	}
	...
	created := session.reached(PhaseControlBucketCreated)
	claimed := session.reached(PhaseControlBucketClaimed)
	switch {
	case exists && !created:
		return outcome, fmt.Errorf("%w: control bucket exists without an operation-bound creation record", ErrPartialBootstrapBlocked)
	case !exists && created:
		return outcome, fmt.Errorf("%w: recorded control bucket is absent", ErrPartialBootstrapBlocked)
	case !exists:
		if !claimed {
			if err := session.append(PhaseControlBucketClaimed, nil, nil); err != nil {
				return outcome, err
			}
		}
		...
		if err := session.append(PhaseControlBucketCreated, nil, nil); err != nil {
			return outcome, err
		}
		state, exists, err = port.DescribeBucket(ctx, identity.Name)
		if err != nil {
	...
	if !state.Versioning {
		// Versioning is applied after the creation record so a failure here
		// converges on retry instead of blocking.
		if err := session.authorizeMutation(); err != nil {
			return outcome, err
		}
		outcome.MutationOccurred = true
		if err := port.EnableVersioning(ctx, identity); err != nil {
			return outcome, err
		}
		state, exists, err = port.DescribeBucket(ctx, identity.Name)
		if err != nil || !exists {
			return outcome, fmt.Errorf("%w: control bucket vanished during convergence", ErrPartialBootstrapBlocked)
		}
	}
...
func checkControlBucketState(state BucketState, identity BucketIdentity) error {
	if state.Identity != identity || !state.UniformBucketLevelAccess || state.PublicAccessPrevention != PublicAccessPreventionEnforced ||
		...
		state.SoftDeleteSeconds != ControlSoftDeleteSeconds {
...
func (executor *StorageExecutor) bucketIAM(ctx context.Context, envelope BootstrapEnvelopeV1, port StorageStepPort, audit, controlBucket BucketIdentity, outcome StepOutcome) (StepOutcome, error) {
	...
	for _, binding := range CompensationBindings(policy.Bindings, preexisting) {
		if err := envelope.validAt(executor.clock().UTC()); err != nil {
			return outcome, err
		}
		outcome.MutationOccurred = true
		if err := port.AddBinding(ctx, identities[binding.Bucket], binding); err != nil {
			// An ambiguous failure may have applied the grant: re-observe and
			// record it as operation-created so compensation can remove it.
			if after, observeErr := port.GetBindings(ctx, identities[binding.Bucket]); observeErr == nil && containsBinding(after, binding) {
	...
	for name, identity := range identities {
		...
		if !bindingsEqualPolicy(after, policy.Bindings, name) {
...
func containsBinding(values []BucketBinding, wanted BucketBinding) bool {
...
// bindingsEqualPolicy requires the observed bindings for bucket to equal the
// rendered set exactly: every rendered binding present and nothing else.
func bindingsEqualPolicy(observed, rendered []BucketBinding, bucket string) bool {
	expected := make(map[BucketBinding]struct{})
...
func (executor *StorageExecutor) seedControl(ctx context.Context, envelope BootstrapEnvelopeV1, port StorageStepPort, controlBucket BucketIdentity, outcome StepOutcome) (StepOutcome, error) {
	store := NewPortControlStore(port, controlBucket)
	seeds, err := SeedControlStore(ctx, store, envelope)
	if err != nil {
		// The preflight is read-only; only a failure after it may have
		// created an object, which is reported conservatively as a mutation.
		outcome.MutationOccurred = !errors.Is(err, ErrSeedIncompatible) && !errors.Is(err, ErrApprovedPolicyMismatch)
		return outcome, err
	}
	...
	// Re-observe every seed after the mutation phase: generations must equal
	// the recorded outcomes and contents must still be compatible.
	expected, err := SeedObjects(envelope)
	if err != nil {
		return outcome, err
	}
	for index, seed := range expected {
		object, err := store.Read(ctx, seed.Name)
		if err != nil {
			return outcome, err
		}
		if object.Descriptor != seeds[index].Descriptor {
			return outcome, fmt.Errorf("%w: seed %s changed after seeding", ErrStepVerificationFailed, seed.Name)
		}
		if err := validateExistingSeed(seed, object.Content, envelope); err != nil {
			return outcome, fmt.Errorf("%w: %v", ErrStepVerificationFailed, err)
		}
	}
...
func (executor *StorageExecutor) lockRoundTrip(ctx context.Context, envelope BootstrapEnvelopeV1, port StorageStepPort, controlBucket BucketIdentity, input LockRoundTripInput, outcome StepOutcome) (StepOutcome, error) {
	...
	if err := rejectDuplicateKeys(current.Content); err != nil {
		return outcome, fmt.Errorf("%w: lock record is ambiguous", ErrStepVerificationFailed)
	}
	if err := decodeStrictSeed(current.Content, &record); err != nil || record.Schema != LockRecordSchemaV1 ||
		...
		record.Holder != nil || record.AcquiredAt != nil || record.HeartbeatAt != nil || record.LeaseUntil != nil ||
		record.OperationID != "" || record.PlanID != "" || record.WorkflowID != "" {
		return outcome, fmt.Errorf("%w: lock is not a clean released record", ErrStepVerificationFailed)
	}
	...
	// Record the held generation immediately so a failure below leaves the
	// caller with what a compensating release needs.
	outcome.LockGenerations[0] = acquired.Generation
	...
	outcome.LockGenerations[1] = released.Generation
	outcome.Verified = true
	return outcome, nil
}

// LockLeaseSeconds is the ARCHITECTURE lock-protocol constant
// (`control.lockLeaseSeconds` = 300). The compiled desired state does not carry
// a manifest override; see the record file for the proposed delta.
const LockLeaseSeconds = 300

// ControlSoftDeleteSeconds is the ARCHITECTURE control-bucket soft delete (30 d).
const ControlSoftDeleteSeconds int64 = 30 * 24 * 60 * 60
