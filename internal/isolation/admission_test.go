...
func TestOpenHarnessRequiresFreshExactUsableT8ForTests(t *testing.T) {
	...
	request := validOpenTestAdmission(open)
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			value := validOpenTestAdmission(open)
			test.mutate(&value)
	...
	if err := isolation.AdmitHarnessAction(drifted, validOpenTestAdmission(drifted)); !errors.Is(err, isolation.ErrHarnessAdmissionDenied) {
		t.Fatalf("AdmitHarnessAction(unusable TEST-I) error = %v; want ErrHarnessAdmissionDenied", err)
	}
	unrelated := validOpenTestAdmission(drifted)
...
func validPendingAdmission(state isolation.HarnessStateV1) isolation.HarnessAdmissionRequest {
	...
	return isolation.HarnessAdmissionRequest{
		...
		Expected: validHarnessExpectation(state), Now: seed.ApprovedAt.Add(30 * time.Minute),
	}
}

func validOpenTestAdmission(state isolation.HarnessStateV1) isolation.HarnessAdmissionRequest {
	evidence := validT8Evidence()
	return isolation.HarnessAdmissionRequest{
		Action:                isolation.HarnessActionIntegrationTest,
		T8ObservationRevision: evidence.Revision,
		Expected:              validHarnessExpectation(state), Now: evidence.ObservedAt.Add(time.Minute),
	}
}

func validHarnessExpectation(state isolation.HarnessStateV1) isolation.HarnessStateExpectation {
	return isolation.HarnessStateExpectation{
		ProjectID: state.ProjectID(), Environment: state.Environment(), EnvironmentClass: state.EnvironmentClass(),
		ManifestHash: state.ManifestHash(), ControlRecordGeneration: state.ControlRecordGeneration(),
		Resources: state.Resources(), CleanupCapabilities: state.CleanupCapabilities(),
	}
}
